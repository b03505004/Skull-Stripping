...
def getDataMinc(fileName, z, z_half, original):
...
def getLabelMinc(fileName, z, z_half, label):
    ...
    for i in range(temp3.shape[1]):
        label.append(np.flip(temp3[:, i, :].reshape(256, 256, 1), 0))
...
def getTorchX(z, z_half, original, isVal):
...
def getTorchLabel(z, z_half, original, isVal):
    for i in range(originalMinc.shape[0]):
        mi = i%(256+2*z_half)
        if ((mi-z_half)<0) or ((mi+z_half)>=(256+2*z_half)):
            continue
...
def getTorchXIBSR(z, z_half, original, isVal):
...
def getTorchLabelIBSR(z, z_half, original, isVal):
...
def getTorchXIBSR20(z, z_half, original, isVal):
    if isVal:
        val_x2.append([])
    for i in range(original.shape[0]):
        ...
        if ((mi-z_half)<0) or ((mi+z_half)>=(original.shape[0])):
            ...
            if (isVal):
                val_x2[-1].append(torch.from_numpy(np.reshape(original[i-z_half : i+z_half+1,...], (1,1,z,original.shape[1],256))).float())
...
def getTorchLabelIBSR20(z, z_half, label, isVal):
    if isVal:
        val_label2.append([])
...
def getTorchXLPBA40(z, z_half, original, isVal):
    if isVal:
        val_x2.append([])
    for i in range(original.shape[0]):
        mi = i%(original.shape[0])
...
def getTorchLabelLPBA40(z, z_half, label, isVal):
    if isVal:
        val_label2.append([])
    for i in range(label.shape[0]):
        mi = i%(label.shape[0])
...
if dataset=="transfer":
    ...
    for i, mFile in enumerate(minc20Files):
        ...
        if mFile.find('crisp')!=-1:
            getLabelMinc('../minc/20/'+mFile, z, z_half, label1)
...
for epoch in range(epochs):  # loop over the dataset multiple times
    ...
    torch.save(net.state_dict(), './models/'+dataset+'_z'+str(z)+'_k'+str(k)+'_epo'+str(epoch+plus_epo+1)+'.pt')
    if(v>last_val):
        early_stopping = 0
        last_val = v
    else:
    ...
    if early_stopping2==0:
        torch.save(net.state_dict(), './models/'+dataset+'_z'+str(z)+'_k'+str(k)+'_epo'+str(epoch+plus_epo+1)+'best.pt')

...
dataset_trained = sys.argv[1]
dataset_to_predict = sys.argv[2]
zs = [1, 3, 5, 7]
...
def val_new(net, val_label, val_x, z, z_half):
    ...
    for b,brain in enumerate(val_label):
        ...
        for i in range(len(brain)):
            ...
            #plt.imsave(arr=val_x[b][i].numpy().reshape(z,-1,256)[z_half,...], cmap='gray', fname='./test/'+str(b)+'_'+str(i)+'_'+'input.jpg')
            #plt.imsave(arr=out, cmap='gray', fname='./test/'+str(b)+'_'+str(i)+'_'+'b4.jpg')
            out = np.reshape(np.where(out>=0.5, 13, 7), (-1,256))
            #plt.imsave(arr=out, cmap='gray', fname='./test/'+str(b)+'_'+str(i)+'_'+'.jpg')
...
def val_ensemble(net, val_label, val_x, zl):
    ...
    for b,brain in enumerate(val_label):
        ...
        for i in range(len(brain)):
            ...
            plt.imsave(arr=val_x[0][b][i].numpy().reshape(-1,256), cmap='gray', fname='./test/'+str(b)+'_'+str(i)+'_'+'input.jpg')
            plt.imsave(arr=out, cmap='gray', fname='./test/'+str(b)+'_'+str(i)+'_'+'b4.jpg')
    ...
    for k in ks:
        ...
        if dataset_to_predict=="ibsr20":
            for brain_num in KK:
                ...
                getTorchX(zs[i], z_halfs[i], temp_original, xtemp)
        ...
        temp_net = fcn(z)
        temp_net.load_state_dict(torch.load('./models/best/'+dataset_trained+'_z'+str(z)+'_k'+str(k)+'_sigmoid_best.pt'))
        nets.append(temp_net)
        
        
        d,j,s1,s2,c,s3 = val_new(temp_net, labels, xtemp, z, z_halfs[i])
        xs.append(xtemp)
        dice += d
        jaccard += j
        sensitivity += s1

    ...
    for b,brain in enumerate(val_label):
        ...
        for i in range(len(brain)):
            ...
            try:
                ...
                thresh = 255
            plt.imsave(arr=val_x[b][i].numpy().reshape(z,-1,256)[z_half,...], cmap='gray', fname='./plot/'+str(b)+'_'+str(i)+'_'+'input.jpg')
            plt.imsave(arr=out, cmap='gray', fname='./plot/'+str(b)+'_'+str(i)+'_'+'b4.jpg')
            out = np.reshape(np.where(out>=thresh, 13, 7), (-1,256))
            plt.imsave(arr=out, cmap='gray', fname='./plot/'+str(b)+'_'+str(i)+'_'+'.jpg')
            lab = np.reshape(np.where(brain[i]==label_val, 3, 1), (-1,256))
            plt.imsave(arr=np.reshape(lab,(-1,256)), cmap='gray', fname='./plot/'+str(b)+'_'+str(i)+'_'+'lab.jpg')
            temp = out+lab
            #print(temp)
...
for i,z in enumerate(zs):
    ...
    for k in ks:
        ...
        modelTotest = sys.argv[4]
        temp_net.load_state_dict(torch.load('./models/lpba_z'+str(zs[0])+'_k'+str(ks[0])+'_epo'+modelTotest+'.pt'))

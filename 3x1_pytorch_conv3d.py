...
def getLabel(fileName):
    ...
    for i in range(z_half, int(label_.shape[0]/2)-z_half):
        newL[i,...] = np.where(scipy.misc.imresize(label_[2*i,:,:], 0.5, interp='nearest')!=0, label_val, 0)
...
print(len(train_label), train_label[0].shape, len(val_label))
print(len(train_x), train_x[0].shape, len(val_x))
...
def val():
    ...
    for i in range(len(val_label)):
        ...
        try:
            ...
            thresh = 255
        #print(thresh)
        plt.imsave(arr=out, cmap='gray', fname='./load keras/'+str(i)+'_'+'b4.jpg')
        out = np.reshape(np.where(out>=thresh, 1, 0), (256,256))
        #print(out)
        plt.imsave(arr=out, cmap='gray', fname='./load keras/'+str(i)+'_'+'.jpg')
        lab = np.reshape(np.where(val_label[i]==label_val, 1, 0), (256,256))
        plt.imsave(arr=np.reshape(lab,(256,256)), cmap='gray', fname='./load keras/'+str(i)+'_'+'lab.jpg')

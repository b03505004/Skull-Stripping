...
def getDataLPBA40Wavelet(fileName, z, z_half, original):
    dtype = np.dtype('<u2')
    fid = open(fileName, 'rb')
    xData = np.fromfile(fid, dtype)
    fid.close()
    ...
    #print(temp.shape)
    for i in range(temp.shape[1]):
        cA, (cH, cV, cD) = pywt.dwt2(np.flip(temp[:, i, :], 0), 'haar')
        original.append((cH + cV + cD)/2.0)
    ...
    for i in range(original.shape[0]):
        ...
        if ((mi-z_half)<0) or ((mi+z_half)>=(original.shape[0])):
            ...
            if (isVal):
                ...
                #plt.imsave(arr=original[i, ...], cmap='gray', fname='./all/'+str(181*20+i)+'_x.jpg')
def getTorchXWT(z, z_half, original, isVal, train_x, val_x):
    if isVal:
        val_x.append([])
    for i in range(original.shape[0]):
        mi = i%(original.shape[0])
        if ((mi-z_half)<0) or ((mi+z_half)>=(original.shape[0])):
            ...
            if (isVal):
                val_x[-1].append(torch.from_numpy(np.reshape(original[i-z_half : i+z_half+1,...], (1,1,z,original.shape[1],128))).float())
                #plt.imsave(arr=original[i, ...], cmap='gray', fname='./all/'+str(181*20+i)+'_x.jpg')
            else:
                train_x.append(torch.from_numpy(np.reshape(original[i-z_half : i+z_half+1,...], (1,1,z,original.shape[1],128))).float())
                #plt.imsave(arr=original[i, ...], cmap='gray', fname='./all/'+str(181*20+i)+'_x.jpg')
...
#**********************************LPBA**********************************
    ...
    for brain_num in ind:
        ...
        #getDataLPBA40Wavelet('../lpba/lpba_img/S'+brain_num+'.native.mri.img', z, z_half, originalTC)
        getDataLPBA40Wavelet('../lpba/lpba_img/S'+brain_num+'.native.mri.img', 1, 0, originalTC)
        ...
        getTorchXWT(1, 0, originalTC, isVal, train_xtc, val_xtc)
        #getTorchXWT(z, z_half, originalTC, isVal, train_xtc, val_xtc)
...
class fcn(nn.Module):
    def __init__(self):
        ...
        #wavelet part
        self.c0tc = nn.Conv3d(1, 32, kernel_size=(1,1,1), stride=(1,1,1))
        #self.c0tc = nn.Conv3d(1, 32, kernel_size=(z,1,1), stride=(1,1,1))
        self.bn_0tc     = nn.BatchNorm3d(32)
...
if sys_argv_len>4:
    if sys.argv[4] == 'load':
        ...
        for i in range(plus_epo//step_size):
            lr*=gamma
        print('lr:', lr)
        net.load_state_dict(torch.load('./models/'+dataset+'_z'+str(z)+'_k'+str(k)+'_epo'+str(plus_epo)+'_wtz.pt'))
...
for epoch in range(epochs):  # loop over the dataset multiple times
    ...
    #v2 = val2(net, val_label, val_x, val_xtc)
    torch.save(net.state_dict(), './models/'+dataset+'_z'+str(z)+'_k'+str(k)+'_epo'+str(epoch+plus_epo+1)+'_wtz.pt')
    ...
    if early_stopping2==0:
        torch.save(net.state_dict(), './models/best/'+dataset+'_z'+str(z)+'_k'+str(k)+'_wtz_best.pt')
